...
export function BranchGraph({ branches }: BranchGraphProps) {
  ...
  // Dynamically map API branch names to visual branch lines
  const activeBranches = branches?.filter((b) => b.status === "active") ?? [];
  const branchName1 = activeBranches[0]?.name ?? "feature/auth-refactor";
  const branchName2 = activeBranches[1]?.name ?? "fix/token-expiry";
    ...
    <div className="flex flex-col rounded border border-border bg-card overflow-hidden h-full">
      ...
      <div className="flex-1 overflow-hidden p-2">
        ...
        >
          ...
          {/* branch 1 line */}
          <path
            d={`M ${positions["C2"]} 40 Q ${positions["C2"] + 10} 80 ${
              positions["C2"] + 20
            } 80 L ${positions["C4"] - 10} 80 Q ${positions["C4"]} 80 ${positions["C4"]} 40`}
            fill="none"
            stroke="#a371f7"
            strokeWidth="1.5"
            strokeOpacity="0.6"
          ...
          {/* branch 2 line */}
          <path
            d={`M ${positions["C3"]} 40 Q ${positions["C3"] + 10} 110 ${
              positions["C3"] + 20
            } 110 L ${positions["C4"] - 10} 110 Q ${positions["C4"]} 110 ${positions["C4"]} 40`}
            fill="none"
            stroke="#3fb950"
            strokeWidth="1.5"
            strokeOpacity="0.6"
          ...
          {/* Branch labels */}
          <text x="22" y="36" fill="#388bfd" fontSize="9" fontFamily="JetBrains Mono, monospace">
            main
          </text>
          <text
            x={positions["C2"] + 25}
